# Attach documentation to a product

The example below shows how to upload a document and attach it to the selected product.
...
After that, we create an identifier for the previously uploaded image with the help of [Document Resource](../../api/document.md), which can be obtained most easily by sending the following request.
...
With the Document resource ID, we can set the name and language of the uploaded document using [Document Description Resource](../../api/document_description.md). The language can be queried using [Language Resource](../../api/language.md). Note: **filename** and **mask** do not have to be the same.
...
If all of this is in place, we can attach the uploaded document to the desired product with the help of [Product Document Relation Resource](../../api/product_document_relation.md). We can query the product using [Product Resource](../../api/product.md).
